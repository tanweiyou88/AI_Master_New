...
# 			image,
# 			os.path.join(resultPath_EnhancedResults, 'enhanced_image_{}.jpg'.format( IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] - sample_size + count + 1) )
# 		)
...
def IQA_metrics_calculation(IQA_metrics_data): # We define IQA_metrics_data as the input argument, so that the results related to IQA_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	...
	IQA_metrics_data['average_ssim'] = IQA_metrics_data['epoch_accumulate_ssim'] / IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']

	# MAE part
	batch_average_mae = mae(enhanced_image, ori_image).item()
	IQA_metrics_data['epoch_accumulate_mae'] += batch_average_mae * sample_size
	IQA_metrics_data['average_mae'] = IQA_metrics_data['epoch_accumulate_mae'] / IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']
	...
	IQA_metrics_data['average_lpips'] = IQA_metrics_data['epoch_accumulate_lpips'] / IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']
...
def ComputationComplexity_metrics_calculation(ComputationalComplexity_metrics_data): # We define ComputationalComplexity_metrics_data as the input argument, so that the results related to ComputationalComplexity_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	...
	ComputationalComplexity_metrics_data['average_val_runtime'] = ComputationalComplexity_metrics_data['accumulate_val_batch_duration_forCompleteTraining'] / (IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] * config.num_epochs)
...
### **Part 3: Training**
	...
	for epoch in range(config.num_epochs):
		...
		IQA_metrics_data = {'epoch':0, 'average_psnr': 0, 'average_ssim': 0, 'average_mae': 0, 'average_lpips': 0, 'epoch_accumulate_number_of_val_input_samples_processed': 0, 'epoch_accumulate_psnr': 0, 'epoch_accumulate_ssim': 0, 'epoch_accumulate_mae': 0,'epoch_accumulate_lpips': 0}
