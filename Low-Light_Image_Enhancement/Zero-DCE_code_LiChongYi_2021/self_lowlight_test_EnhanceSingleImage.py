...
def lowlight(image_path):
	...
	DCE_net.load_state_dict(torch.load('D:/AI_Master_New/Low-Light_Image_Enhancement/Zero-DCE_code_LiChongYi_2021/snapshots/Epoch99.pth'))
	start = time.time()
	_,enhanced_image,_ = DCE_net(data_lowlight)

	end_time = (time.time() - start)
	print(end_time)
	# image_path = image_path.replace('try_image_pairs','result')
	image_path = image_path.replace('low-light','enhanced')
...
# test_images
	with torch.no_grad():
		# filePath = 'D:/AI_Master_New/early_blight_test/' # The absolute path that stores the test data
		# image = filePath + 'early_blight_test_1_low-light.JPG'

		# filePath = 'D:/AI_Master_New/late_blight_test/' # The absolute path that stores the test data
		# image = filePath + 'late_blight_test_0_low-light.JPG'

		filePath = 'D:/AI_Master_New/healthy_test/' # The absolute path that stores the test data
		image = filePath + 'healthy_test_1_low-light.JPG'
