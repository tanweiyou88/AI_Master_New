...
def lowlight(image_path):
	...
	data_lowlight = data_lowlight.resize((50,50), Image.LANCZOS)
	...
	enhanced_image_cpu = enhanced_image.cpu() # move the tensor from cuda back to cpu. Because we cannot directly convert the tensor on cuda into numpy array
	# print(enhanced_image_cpu)

	
	t_np = enhanced_image_cpu.numpy() #convert to Numpy array
	# print(t_np)
	t_np_reshape = t_np.reshape(t_np.shape[0],-1) # reshape into 2D array
	# pd.set_option('display.max_columns', None)
	df = pd.DataFrame(t_np_reshape) #convert the numpy array into a dataframe
	# print("df:",df)
	...
	csv_result_filepath = os.path.join("D:/AI_Master_New/Low-Light_Image_Enhancement/Zero-DCE_code_LiChongYi_2021/data/result/self_CompileMetrics/self_CompileMetrics_csvFile",csv_result_filename) # Create the path to the csv that stores the metrics data
...
# test_images
	...
	with torch.no_grad():
		...
		# 			if image == "D:/AI_Master_New/Low-Light_Image_Enhancement/Zero-DCE_code_LiChongYi_2021/data/test_data/DICM/02.jpg":
		# 				# image = image
		...
		image = "D:/AI_Master_New/Low-Light_Image_Enhancement/Zero-DCE_code_LiChongYi_2021/data/test_data/DICM/02.jpg"
		print(image) # Show the absolute path of that image
		lowlight(image) # Perform image processing and image enhancement using Zero-DCE on that image 
