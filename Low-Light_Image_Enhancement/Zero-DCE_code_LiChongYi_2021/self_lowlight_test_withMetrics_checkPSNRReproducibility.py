# The PSNR will be constant for the same images at each time running this script, only if "torch.manual_seed(your_favourite_number)" is defined to initialize the PyTorch RNG state to generate the base seed that used by each worker of Dataloder to generate a series of shuffled indices that is reproducible/deterministic. 
import torch
...
import argparse
import time
import dataloader_CheckIndices
import model
import numpy as np
...
# def _init_fn(worker_id): # Initialize workers' seed, used as the argument for worker_init_fn of dataloader
	...
	with torch.no_grad():
		...
		for testfile_name in test_list: # For each subfolder inside the test_data folder (DCIM subfolder first, then only LIME subfolder)
			if testfile_name == "DICM": # Only takes the DICM test data available in testPath
				...
				test_dataset = dataloader_CheckIndices.lowlight_loader(test_path_withFolderName) # Create custom test dataset: Take the images available in test_path, then preprocess them, and convert them into a tensor
		...
		# 	enhanced_image_cpu = enhanced_image.cpu()
		# 	enhanced_image_cpu_numpy = enhanced_image_cpu.numpy() #convert to Numpy array
		# 	enhanced_image_cpu_numpy_reshape = enhanced_image_cpu_numpy.reshape(1,-1) # reshape into 2D array
			
			
		# 	LLIE_results_pool.append(enhanced_image_cpu_numpy_reshape) # record the LLIE outputs at each iteration
		...
		# # Record Zero-DCE results
		# csv_result_filename = "Zero-DCE-Outputs"+ '-' + current_date_time_string + '.csv' # Create the filename of the csv that stores the metrics data
		# csv_result_filepath = os.path.join(resultPath,resultPath_subfolder,resultPath_subfolder_csv,csv_result_filename) # Create the path to the csv that stores the metrics data
		
		# # t_np = LLIE_results_pool.numpy() #convert to Numpy array
		# # squeeze_list = [item[0,2] for item in LLIE_results_pool] # Squeeze the list
		# squeeze_list = np.squeeze(LLIE_results_pool, axis=1) # Squeeze the list
		# df = pd.DataFrame(squeeze_list) #convert the numpy array into a dataframe
		# df.to_csv(csv_result_filepath,index=False) #save the dataframe into a CSV file

		
		# with open(csv_result_filepath, 'w', newline='') as csvfile: # Create and open an empty csv file at the path of csv_result_filepath with write mode, later we append different dictionaries of metric data as required. Now the opened csv file is called csvfile object.
		# 	writer = csv.writer(csvfile, delimiter=' ') # The writer (csv.DictWriter) takes the csvfile object as the csv file to write and IQA_metrics_data.keys() as the elements=keys of the header
		# 	writer.writerows(LLIE_results_pool) # The writer writes the header on the csv file
