...
def PSNR(preds, target, data_range=torch.tensor(1.0), base =10.0): # self-written function to return the total PSNR of a batch of images
	...
	psnr_sum_values_batch = 0
	# print("target.shape",target.shape)
	# print("preds.shape",preds.shape)
	target = target.cpu().numpy().astype(float)
	target = np.transpose(target, (0, 2, 3, 1)) # switch dimension order
	preds = preds.cpu().numpy().astype(float)
	preds = np.transpose(preds, (0, 2, 3, 1)) # switch dimension order
	# print("target.shape",target.shape)
	# print("preds.shape",preds.shape)
	for index in range(len(target)):
		# pred, target = preds[index].cpu().numpy().astype(float), target[index].cpu().numpy().astype(float)
		pred, targ = preds[index], target[index]
		# print("pred.shape",pred.shape)
		# print("targ.shape",targ.shape)
		value = psnr(pred, targ, data_range=1.0)
		psnr_sum_values_batch += value
		# psnr_values_batch.append(value)
		# batch_sum_psnr = np.sum(psnr_values_batch)
		# average_psnr_batch = np.mean(psnr_values_batch) if psnr_values_batch else 0
	
	# return batch_sum_psnr
	return psnr_sum_values_batch

def SSIM(preds, target):
	ssim_sum_values_batch = 0
	# print("target.shape",target.shape)
	# print("preds.shape",preds.shape)
	target = target.cpu().numpy().astype(float)
	...
	for index in range(len(target)):
		...
		# print("pred.shape",pred.shape)
		# print("targ.shape",targ.shape)
		value = ssim(pred, targ, data_range=1.0, channel_axis=2)
		ssim_sum_values_batch += value
		# psnr_values_batch.append(value)
		# batch_sum_psnr = np.sum(psnr_values_batch)
		# average_psnr_batch = np.mean(psnr_values_batch) if psnr_values_batch else 0
	
	# return batch_sum_psnr
	return ssim_sum_values_batch

def MAE(preds, target):
	mae_sum_values_batch = 0
	# print("target.shape",target.shape)
	# print("preds.shape",preds.shape)
	target = target.cpu().numpy().astype(float)
	target = np.transpose(target, (0, 2, 3, 1))
	...
	for index in range(len(target)):
		...
		# print("pred.shape",pred.shape)
		# print("targ.shape",targ.shape)
		value = np.mean(np.abs(pred - targ))
		mae_sum_values_batch += value
		# psnr_values_batch.append(value)
		# batch_sum_psnr = np.sum(psnr_values_batch)
		# average_psnr_batch = np.mean(psnr_values_batch) if psnr_values_batch else 0
	
	# return batch_sum_psnr
	return mae_sum_values_batch
...
def IQA_metrics_calculation(IQA_metrics_data): # We define IQA_metrics_data as the input argument, so that the results related to IQA_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	...
	# SSIM part
	batch_sum_ssim = SSIM(enhanced_image, ori_image)
	IQA_metrics_data['epoch_accumulate_ssim'] += batch_sum_ssim 
	# batch_sum_ssim = ssim(enhanced_image, ori_image)
	...
	# MAE part
	batch_sum_mae = MAE(enhanced_image, ori_image)
	IQA_metrics_data['epoch_accumulate_mae'] += batch_sum_mae 
...
# **Initialize folders
	...
	csv_TrainingResult_filename = "{}-{}-{}-TrainResultsHistory.csv".format(current_date_time_string, config.model_name, config.dataset_name) # Create the filename of the csv that stores the metrics data
	...
	csv_ValidationResult_filename = "{}-{}-{}-ValResultsHistory.csv".format(current_date_time_string, config.model_name, config.dataset_name) # Create the filename of the csv that stores the metrics data
...
### **Part 3: Training**
	...
	for epoch in range(config.num_epochs):
		...
		for iteration, (ori_image, LL_image) in enumerate(train_bar):
			...
			loss_TV = 20*L_TV(A) # Calculate the illumination smoothness loss of this batch of samples
			loss_spa = torch.mean(L_spa(enhanced_image, LL_image)) # Calculate the spatial consistency loss of this batch of samples
			loss_col = 0.5*torch.mean(L_color(enhanced_image)) # Calculate the color constancy loss of this batch of samples
			loss_exp = torch.mean(L_exp(enhanced_image)) # Calculate the exposure control loss of this batch of samples
