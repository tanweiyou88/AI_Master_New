...
def load_data(config):
	...
	train_dataset = dataloader_WithPair_EMP.LLIEDataset(config.train_GroundTruth_root, config.train_Input_root, config.image_square_size)
	train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.train_batch_size, shuffle=True,
												num_workers=config.num_workers, pin_memory=True)
	print('11) For validation set:')
	val_dataset = dataloader_WithPair_EMP.LLIEDataset(config.val_GroundTruth_root, config.val_Input_root, config.image_square_size)
...
def save_model(epoch, path, net, optimizer, key): # this function saves model checkpoints
	...
	if key == 0:
		...
		torch.save({'epoch': epoch, 'model_state_dict': net.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'loss': loss}, # save the checkpoints that has the best epoch_average_lpips
					f=os.path.join(path, '{}-{}-{}-BestEpoAveLoss_checkpoint.pt'.format(current_date_time_string, config.model_name, config.dataset_name)))
...
if __name__ == '__main__':
	...
	parser.add_argument('--dir_store_results', type=str, default="C:/Master_XMUM_usages/AI_Master_New/Experiment_Master_Project_EMP/ZeroDCE_ori_WeightsReferPaper2_EMP") 
...
### **Part 3: Training**
	...
	for epoch in range(config.num_epochs):
		...
		if ((epoch+1) % config.snapshot_iter) == 0: # at every (snapshot_iter)th iteration
			save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=0)	
		...
		with torch.no_grad():
			...
			if  Validation_IQA_metrics_data['epoch_average_psnr'] > best_epoch_validation_average_psnr :
				best_epoch_validation_average_psnr = Validation_IQA_metrics_data['epoch_average_psnr']
				save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=1)	
				
			if  Validation_IQA_metrics_data['epoch_average_ssim'] > best_epoch_validation_average_ssim :
				best_epoch_validation_average_ssim = Validation_IQA_metrics_data['epoch_average_ssim']
				save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=2)

			if  Validation_IQA_metrics_data['epoch_average_mae'] < best_epoch_validation_average_mae :
				best_epoch_validation_average_mae = Validation_IQA_metrics_data['epoch_average_mae']
				save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=3)	
			
			if  Validation_IQA_metrics_data['epoch_average_lpips'] < best_epoch_validation_average_lpips :
				best_epoch_validation_average_lpips = Validation_IQA_metrics_data['epoch_average_lpips']
				save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=4)

			if  Validation_losses_data['epoch_average_loss'] < best_epoch_validation_average_loss :
				best_epoch_validation_average_loss = Validation_losses_data['epoch_average_loss']
				save_model(epoch, resultPath_ModelParametersResults, DCE_net, optimizer, key=5)
