...
class LLIEDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, GroundTruth_root, Input_root, image_size_width, image_size_height):
        ...
        # self.isdemo = isdemo
        self.size_width = image_size_width # The width of each image, in pixel
        self.size_height = image_size_height # The height of each image, in pixel
    ...
    def __getitem__(self, item):
        ...
        ori_image = ori_image.resize((self.size_width,self.size_height), Image.LANCZOS) # Resize the opened image to (self.size,self.size) with Image.ANTIALIAS filter for anti-aliasing, using PIL. Then, replace the opened image with its resized image at the given image's absolute path. **ANTIALIAS was removed in Pillow 10.0.0 (after being deprecated through many previous versions). Now you need to use PIL.Image.LANCZOS or PIL.Image.Resampling.LANCZOS.(This is the exact same algorithm that ANTIALIAS referred to, you just can no longer access it through the name ANTIALIAS.)**
        ...
        ll_image = ll_image.resize((self.size_width, self.size_height), Image.LANCZOS) # Resize the opened image to (self.size,self.size) with Image.ANTIALIAS filter for anti-aliasing, using PIL. Then, replace the opened image with its resized image at the given image's absolute path. **ANTIALIAS was removed in Pillow 10.0.0 (after being deprecated through many previous versions). Now you need to use PIL.Image.LANCZOS or PIL.Image.Resampling.LANCZOS.(This is the exact same algorithm that ANTIALIAS referred to, you just can no longer access it through the name ANTIALIAS.)**
