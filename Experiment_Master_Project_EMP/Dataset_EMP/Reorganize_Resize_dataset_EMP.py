# This script aims to take an original LOL.v2_real_test dataset to reorganize and followed by resizing (primarily to ensure each image is in 'RGB' mode) it.
...
# ** Save all the files according to 2 subfolders only, "Input" OR "Label"
    ...
    for folder_name in tqdm(sorted_folder_list, desc="Reorganizing original dataset"): # For each folder in the sorted folder list. tqdm is used to create the progress bar.
        ...
        # for file in file_list: # For each image in the file list (available inside the current folder)
                ...
                file_filename, file_extension = os.path.splitext(os.path.basename(file)) # Split the file path into (head, tail) pair first, then split the tail component into filename and file extension respectively. Get an absolute path of a file and split it into path and file extension respectively. Ref:https://docs.python.org/3/library/os.path.html#os.path.splitext             
                metadata['file_ori_filename'] = file_filename
                # print("file_filename:", metadata['file_ori_filename'])
                
                GroundTruth_file, = glob.glob(GroundTruth_src_dir + "/" + metadata['file_ori_filename'] + ".*") # Use sequence unpacking method to get the only element from the list. Ref: https://stackoverflow.com/questions/33161448/getting-only-element-from-a-single-element-list-in-python
                ...
                Temp_GroundTruth_file, = glob.glob(temp_destination_GroundTruth_folder_path + "/" + metadata['file_ori_filename'] + ".*") # Get the absolute path of the file available in the Temp_GroundTruth folder, in the destination root folder
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_dir', type=str, default= "C:/Master_XMUM_usages/AI_Master_New/Experiment_Master_Project_EMP/Dataset_EMP/Unprocessed/LOLv2_EMP/Real_captured/Test") # The absolute path of the source dataset
    parser.add_argument('--dst_dir', type=str, default="C:/Master_XMUM_usages/AI_Master_New/Experiment_Master_Project_EMP/Dataset_EMP/Preprocessed_LOLv2-Real-test_Dataset_Part2_EMP") # The absolute path of the destination root folder that saves the rearranged dataset
    parser.add_argument('--source_dataset_name', type=str, default="LOLv2-Real-test") # The name of the source dataset
    ...
    parser.add_argument('--image_size_height', type=int, default=400) # The height size of the input validation images to be resized (in pixel dimension)
    parser.add_argument('--image_size_width', type=int, default=600) # The width size of the input validation images to be resized (in pixel dimension)
    ...
    metadata = {'folder_name':0, 'file_ori_filename': 0, 'image_counter': -1}
    ...
    GroundTruth_src_dir = config.src_dir+"/Normal" # The absolute path of the folder that contains the ground truth files/images, in the source folder
