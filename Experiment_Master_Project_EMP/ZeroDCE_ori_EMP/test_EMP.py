...
def load_data(config):
	...
	print('11) For test set:')
	test_dataset = dataloader_WithPair_latest_EMP.LLIEDataset(config.test_GroundTruth_root, config.test_Input_root, config.test_image_size_width, config.test_image_size_height)
...
def IQA_metrics_calculation(Validation_IQA_metrics_data): # We define Validation_IQA_metrics_data as the input argument, so that the results related to Validation_IQA_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	sample_size = test_LL_image.shape[0] # Get the total number of input samples (img_lowlight.shape[0]) that have been processed by the model in the current iteration, can be equivalent to batch size.
	Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] += sample_size # Update the processed input samples in the current iteration. So that at the end, Validation_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] stores the total number of input samples that have been processed by the model.

	# PSNR part
	batch_sum_psnr = PSNR(test_enhanced_image, test_ori_image) # get the total PSNR of a batch of images
	Test_IQA_metrics_data['epoch_accumulate_psnr'] += batch_sum_psnr
	Test_IQA_metrics_data['epoch_average_psnr_db'] = Test_IQA_metrics_data['epoch_accumulate_psnr'] / Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] # Get the average PSNR of all image pairs the model has gone through. The mathematical concept behinds is: (Summation of PSNR of all image pairs the model has gone through)/[Total batches of images the model has gone through, which is same as the total number of image pairs the model has gone through]).
	Test_IQA_metrics_data['epoch_average_psnr'] = Test_IQA_metrics_data['epoch_average_psnr_db']/48.13 # PSNR value is normalized with 48.13 using min max normalization. For 8-bit image, min max normalization = (inst_value - ori_min_value)/(ori_max_value - ori_min_value); where ori_max_value = 20 log (255/255) - 10 log [(1/255)^2]= 48.13dB,  ori_min_value = 20 log (255/255) - 10 log [(255/255)^2]= 0dB


	# SSIM part
	batch_average_ssim = ssim(test_enhanced_image, test_ori_image).item()
	Test_IQA_metrics_data['epoch_accumulate_ssim'] += batch_average_ssim * sample_size
	Test_IQA_metrics_data['epoch_average_ssim'] = Test_IQA_metrics_data['epoch_accumulate_ssim'] / Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']

	# MAE part
	batch_average_mae = mae(test_enhanced_image, test_ori_image).item()
	Test_IQA_metrics_data['epoch_accumulate_mae'] += batch_average_mae * sample_size
	Test_IQA_metrics_data['epoch_average_mae'] = Test_IQA_metrics_data['epoch_accumulate_mae'] / Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']
	
	# LPIPS part
	batch_average_lpips = lpips(test_enhanced_image, test_ori_image).item()
	Test_IQA_metrics_data['epoch_accumulate_lpips'] += batch_average_lpips * sample_size
	Test_IQA_metrics_data['epoch_average_lpips'] = Test_IQA_metrics_data['epoch_accumulate_lpips'] / Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']

	# IQA score part
	Test_IQA_metrics_data['epoch_average_IQAScore'] = Test_IQA_metrics_data['epoch_average_psnr'] + Test_IQA_metrics_data['epoch_average_ssim'] - Test_IQA_metrics_data['epoch_average_mae'] - Test_IQA_metrics_data['epoch_average_lpips']



def ComputationComplexity_metrics_calculation(Test_ComputationalComplexity_metrics_data): # We define Validation_ComputationalComplexity_metrics_data as the input argument, so that the results related to Validation_ComputationalComplexity_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	...
	# Test_ComputationalComplexity_metrics_data['average_val_runtime_forCompleteOperations'] = Test_ComputationalComplexity_metrics_data['accumulate_val_batch_duration_forCompleteOperations'] / (Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'] * config.num_epochs)
	Test_ComputationalComplexity_metrics_data['average_test_runtime_forCompleteOperations'] = Test_ComputationalComplexity_metrics_data['accumulate_test_batch_duration_forCompleteOperations'] / Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed']
	...
	reference_input_shape = (batch_size, test_LL_image.shape[1], test_LL_image.shape[2], test_LL_image.shape[3]) # (reference batch size for calflops, channel numbers of each input sample, height dimension of each input sample, width dimension of each input sample)
	Test_ComputationalComplexity_metrics_data['FLOPs'], Test_ComputationalComplexity_metrics_data['MACs'], Test_ComputationalComplexity_metrics_data['trainable_parameters'] = calculate_flops(model=DCE_net,  # The model created
...
# 	df_TrainingLossesResult = pd.read_csv(csv_TestLossesResult_filepath) # convert the csv file into a Dataframe
...
# 	df_ValidationLossesResult = pd.read_csv(csv_TestLossesResult_filepath) # convert the csv file into a Dataframe
...
# 	with open(csv_TestLossesResult_filepath, 'a', newline='') as csvfile: # Open that csv file at the path of csv_ValidationResult_filepath with append mode, so that we can append the data of Validation_ComputationalComplexity_metrics_data dictionary to that csv file.
...
# 	df_ValidationResult = pd.read_csv(csv_TestIQAResult_filepath) # convert the csv file into a Dataframe
...
# 	with open(csv_TestIQAResult_filepath, 'a', newline='') as csvfile: # Open that csv file at the path of csv_ValidationResult_filepath with append mode, so that we can append the data of Validation_ComputationalComplexity_metrics_data dictionary to that csv file.
...
# **Subpart 1: Select the device for computations**
	...
	print('9) The images in train set will be resized to (height x width, pixel dimensions): {} x {}'.format(config.test_image_size_height, config.test_image_size_width))
...
# **Initialize folders
	...
	csv_TestLossesResult_filename = "{}-{}-{}-TestLossesHistory.csv".format(current_date_time_string, config.model_name, config.dataset_name) # Create the filename of the csv that stores the metrics data
	csv_TestLossesResult_filepath = os.path.join(resultPath_csv, csv_TestLossesResult_filename) # Create the path to the csv that stores the metrics data

	# To record validation results 
	csv_TestIQAResult_filename = "{}-{}-{}-TestIQAHistory.csv".format(current_date_time_string, config.model_name, config.dataset_name) # Create the filename of the csv that stores the metrics data
	csv_TestIQAResult_filepath = os.path.join(resultPath_csv, csv_TestIQAResult_filename) # Create the path to the csv that stores the metrics data
	...
	csv_Testconfiguration_filename = "{}-{}-{}-TestConfiguration.csv".format(current_date_time_string, config.model_name, config.dataset_name) # Create the filename of the csv that stores the metrics data
	csv_Testconfiguration_filepath = os.path.join(resultPath_csv, csv_Testconfiguration_filename) # Create the path to the csv that stores the metrics data

	# For the folder that stores the grids of input-output-GroundTruth image groups involved in metric calculations
	resultPath_subfolder_sample_test_output_folder = "TestSampleValOutput" # Define the folder that stores the validation output samples
	sample_test_output_folder = os.path.join(resultPath, resultPath_subfolder_sample_test_output_folder) # Create a new path by changing a part of resultPath
	os.makedirs(sample_test_output_folder, exist_ok=True) # Ensure the path that stores the validation output samples is created
	...
	with open(csv_Testconfiguration_filepath, 'w', newline='') as csvfile: 
		writer = csv.writer(csvfile, delimiter=',')
		writer.writerow(['-------LLIE model testing configurations-------'])
...
# **Subpart 2: Initialize training and validation dataset**
	...
	test_loader, test_number = load_data(config)
...
# **Subpart 6: Initialize the IQA metric-related functions to calculate IQA results during validation**
	...
	Test_ComputationalComplexity_metrics_data ={'average_test_runtime_forCompleteOperations':0., 'trainable_parameters':0., 'MACs':0., 'FLOPs': 0., 'accumulate_test_batch_duration_forCompleteOperations':0.}
	...
	if len(test_loader) < max_firstNBatches_ImageGroups_VisualizationSample: # len(val_loader) returns the number of batches of the dataset = total number of iterations/batches. len(val_loader.dataset) returns the number samples in the dataset = the number of input-output-GroundTruth image groups.
		max_firstNBatches_ImageGroups_VisualizationSample = len(test_loader) # when the total batches number of the dataset is less than the specified maximum batches number for visualization, takes the total batches number of the dataset as the maximum batches number for visualization

	save_image = None

	with torch.no_grad():
		for iteration, (test_ori_image, test_LL_image) in enumerate(test_bar):
			
			test_ori_image, test_LL_image = test_ori_image.cuda(), test_LL_image.cuda()
			...
			test_enhanced_image_1,test_enhanced_image,test_A = DCE_net(test_LL_image) # Call the model to enhance this batch of samples, by returning each image enhanced at 4th enhancing iteration, each image enhanced at last enhancing iteration, and the 24 curve parameter maps used to enhance this batch of samples at different stages respectively.
			
			test_batch_duration = (time.time() - start) # Get the duration of image enhancement process on the current batch of samples

			test_loss_TV = config.weightage_loss_TV*L_TV(test_A) # Calculate the illumination smoothness loss of this batch of samples
			test_loss_spa = config.weightage_loss_spa*torch.mean(L_spa(test_enhanced_image, test_LL_image)) # Calculate the spatial consistency loss of this batch of samples
			test_loss_col = config.weightage_loss_col*torch.mean(L_color(test_enhanced_image)) # Calculate the color constancy loss of this batch of samples
			test_loss_exp = config.weightage_loss_exp*torch.mean(L_exp(test_enhanced_image)) # Calculate the exposure control loss of this batch of samples
			# Total loss of this batch of samples (best_loss, considering the weightage of each loss contributed to the total loss)
			test_loss =  test_loss_TV + test_loss_spa + test_loss_col + test_loss_exp 

			# The epoch losses for record purpose only
			Test_losses_data['epoch_accumulate_loss'] = Test_losses_data['epoch_accumulate_loss'] + test_loss.item() # epoch_accumulate_loss stores the accumulate loss in an epoch, for record purpose only (not for backpropagation)
			Test_losses_data['epoch_average_loss'] = Test_losses_data['epoch_accumulate_loss']/(iteration+1)
			Test_losses_data['epoch_accumulate_Loss_TV'] = Test_losses_data['epoch_accumulate_Loss_TV'] + test_loss_TV.item() # epoch_accumulate_Loss_TV stores the accumulate Loss_TV in an epoch, for record purpose only (not for backpropagation)
			Test_losses_data['epoch_average_Loss_TV'] = Test_losses_data['epoch_accumulate_Loss_TV']/(iteration+1)
			Test_losses_data['epoch_accumulate_loss_spa'] = Test_losses_data['epoch_accumulate_loss_spa'] + test_loss_spa.item() # epoch_accumulate_loss_spa stores the accumulate loss_spa in an epoch, for record purpose only (not for backpropagation)
			Test_losses_data['epoch_average_loss_spa'] = Test_losses_data['epoch_accumulate_loss_spa']/(iteration+1)
			Test_losses_data['epoch_accumulate_loss_col'] = Test_losses_data['epoch_accumulate_loss_col'] + test_loss_col.item() # epoch_accumulate_loss_col stores the accumulate loss_col in an epoch, for record purpose only (not for backpropagation)
			Test_losses_data['epoch_average_loss_col'] = Test_losses_data['epoch_accumulate_loss_col']/(iteration+1)
			Test_losses_data['epoch_accumulate_loss_exp'] = Test_losses_data['epoch_accumulate_loss_exp'] + test_loss_exp.item() # epoch_accumulate_loss_exp stores the accumulate loss_exp in an epoch, for record purpose only (not for backpropagation)
			Test_losses_data['epoch_average_loss_exp'] = Test_losses_data['epoch_accumulate_loss_exp']/(iteration+1)
			batch_test_sample_size = test_LL_image.shape[0] # Get the total number of input samples (LL_image.shape[0]) that have been processed by the model in the current iteration, can be equivalent to batch size.
			Test_losses_data['epoch_accumulate_number_of_training_input_samples_processed'] += batch_test_sample_size # Update the processed input samples in the current iteration. So that at the end, Training_losses_data['epoch_accumulate_number_of_training_input_samples_processed'] stores the total number of input samples that have been processed by the model.
			...
			Test_ComputationalComplexity_metrics_data['accumulate_test_batch_duration_forCompleteOperations'] += test_batch_duration # Update the accumulate batch duration to the Validation_ComputationalComplexity_metrics_data dictionary
			...
			IQA_metrics_calculation(Test_IQA_metrics_data) 
			...
			if iteration <= (max_firstNBatches_ImageGroups_VisualizationSample - 1):   # before reaching the first max_step number of iterations/batches in each epoch, the val_LL_image (Input), val_enhanced_image (Output), and val_ori_image (GroundTruth) image groups the network has dealt with will be concatenated horizontally together as an image grid. So max_step determines the number of groups will be concatenated horizontally in the image grid. In other words, only the first max_step groups will be chosen as the samples to be concatenated as the image grid to show/visualize the network performance.
				sv_im = torchvision.utils.make_grid(torch.cat((test_LL_image, test_enhanced_image, test_ori_image), 0), nrow=test_ori_image.shape[0])
				if save_image == None:
					save_image = sv_im
				else:
			...
			if iteration == (max_firstNBatches_ImageGroups_VisualizationSample - 1):   # when reaching max_step number of iterations/batches in each epoch, the image grid will be saved on the device. The number of image groups in the image grid = firstNBatches * batch_size during validation.
				torchvision.utils.save_image(
					save_image,
					os.path.join(sample_test_output_folder, '{}-{}-{}-SampleValOutput.jpg'.format(current_date_time_string, config.model_name, config.dataset_name))
				)

			test_bar.set_description_str('Iteration: %d/%d | AccumProcessed_ValSamples: %d | EpochAve_loss: %.4f; EpochAve_IQAScore: %.4f; EpochAve_PSNR: %.4f dB; EpochAve_SSIM: %.4f; EpochAve_MAE: %.4f; EpochAve_LPIPS: %.4f' % (
						iteration + 1, test_number, 
						Test_IQA_metrics_data['epoch_accumulate_number_of_val_input_samples_processed'], 
						Test_losses_data['epoch_average_loss'], Test_IQA_metrics_data['epoch_average_IQAScore'], Test_IQA_metrics_data['epoch_average_psnr_db'],Test_IQA_metrics_data['epoch_average_ssim'], Test_IQA_metrics_data['epoch_average_mae'], Test_IQA_metrics_data['epoch_average_lpips']))
				

		# Record calculated IQA metrics on validation set
		with open(csv_TestIQAResult_filepath, 'w', newline='') as csvfile: # Create and open an empty csv file at the path of csv_ValidationResult_filepath with write mode, later we append different dictionaries of metric data as required. Now the opened csv file is called csvfile object.
			writer = csv.DictWriter(csvfile, fieldnames=Test_IQA_metrics_data.keys()) # The writer (csv.DictWriter) takes the csvfile object as the csv file to write and Validation_IQA_metrics_data.keys() as the elements=keys of the header
			writer.writeheader() # The writer writes the header on the csv file
			writer.writerow(Test_IQA_metrics_data) # The writer writes the data (value) of Validation_IQA_metrics_data dictionary in sequence as a row on the csv file

		# Record calculated losses metrics on validation set
		with open(csv_TestLossesResult_filepath, 'w', newline='') as csvfile: # Create and open an empty csv file at the path of csv_ValidationResult_filepath with write mode, later we append different dictionaries of metric data as required. Now the opened csv file is called csvfile object.
			writer = csv.DictWriter(csvfile, fieldnames=Test_losses_data.keys()) # The writer (csv.DictWriter) takes the csvfile object as the csv file to write and Training_losses_data.keys() as the elements=keys of the header
			writer.writeheader() # The writer writes the header on the csv file
			writer.writerow(Test_losses_data) # The writer writes the data (value) of Validation_IQA_metrics_data dictionary in sequence as a row on the csv file
		
		# **Subpart 13: Record the calculated computation complexity metrics to that csv file**
		ComputationComplexity_metrics_calculation(Test_ComputationalComplexity_metrics_data)
		with open(csv_TestIQAResult_filepath, 'a', newline='') as csvfile: # Open that csv file at the path of csv_ValidationResult_filepath with append mode, so that we can append the data of Validation_ComputationalComplexity_metrics_data dictionary to that csv file.
			writer = csv.DictWriter(csvfile, fieldnames=Test_ComputationalComplexity_metrics_data.keys()) # The writer (csv.DictWriter) takes the csvfile object as the csv file to write and Validation_ComputationalComplexity_metrics_data.keys() as the elements=keys of the header
			writer.writeheader() # The writer writes the header on the csv file
			writer.writerow(Test_ComputationalComplexity_metrics_data)  # The writer writes the data (value) of Validation_ComputationalComplexity_metrics_data dictionary in sequence as a row on the csv file
