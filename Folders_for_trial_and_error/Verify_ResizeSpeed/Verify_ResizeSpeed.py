...
class lowlight_loader(data.Dataset):
	...
	def __init__(self, lowlight_images_path): # The __init__ function is run once when instantiating the Dataset object. We initialize the directory containing the samples (Here, the samples are the images)
		...
		print("Total images:", len(self.train_list))
...
def main(config):
    ...
    print("Resizing original (bigger images with inconsistent dimensions) image dataset into 256x256 images:")
    ori_image_dataset = lowlight_loader(config.ori_image_dataset_path)	# set the absolute path of the training dataset	
        
    ori_image_loader = torch.utils.data.DataLoader(ori_image_dataset, batch_size=config.train_batch_size, shuffle=True, num_workers=config.num_workers, pin_memory=True) # Split the samples in the training dataset into batches/groups/collections of train_batch_size samples. By default, train_batch_size=8, that's means each batch has 8 samples.

    start = time.time() # Get the starting time of image enhancement process on a batch of samples, in the unit of second.

    for epoch in range(config.num_epochs):
        for iteration, img_lowlight in tqdm(enumerate(ori_image_loader)):
            pass

    ori_image_batch_duration = (time.time() - start) # Get the duration of image enhancement process on the current batch of samples
    print("Duration to resize original (bigger images with inconsistent dimensions) image dataset into 256x256 images, for 1 epoch (seconds):", ori_image_batch_duration)
	
    print("\nResizing big (512x512 images) image dataset into 256x256 images:")
    big_image_dataset = lowlight_loader(config.big_image_dataset_path)	# set the absolute path of the training dataset	
        
    big_image_loader = torch.utils.data.DataLoader(big_image_dataset, batch_size=config.train_batch_size, shuffle=True, num_workers=config.num_workers, pin_memory=True) # Split the samples in the training dataset into batches/groups/collections of train_batch_size samples. By default, train_batch_size=8, that's means each batch has 8 samples.

    start = time.time() # Get the starting time of image enhancement process on a batch of samples, in the unit of second.

    for epoch in range(config.num_epochs):
        for iteration, img_lowlight in tqdm(enumerate(big_image_loader)):
            pass

    big_image_batch_duration = (time.time() - start) # Get the duration of image enhancement process on the current batch of samples
    print("Duration to resize big (512x512 images) image dataset into 256x256 images, for 1 epoch (seconds):", big_image_batch_duration)
...
if __name__ == "__main__": 
	...
	# Input Parameters
	parser.add_argument('--ori_image_dataset_path', type=str, default="D:/AI_Master_New/Verify_ResizeSpeed/ori_images/") # Add an argument type (optional argument) named lowlight_images_path. The value given to this argument type must be string data type. If no value is given to this argument type, then the default value will become the value of this argument type.
	parser.add_argument('--big_image_dataset_path', type=str, default="D:/AI_Master_New/Verify_ResizeSpeed/big_images/") # Add an argument type (optional argument) named lowlight_images_path. The value given to this argument type must be string data type. If no value is given to this argument type, then the default value will become the value of this argument type.
	...
	main(config) # Call the train() and passing all the argument values stored in config to train()
