...
def train(config):
	...
	DCE_net = ZTWV_model.enhance_net_nopool().cuda() # Move the model to cuda so that the samples in each batch can be processed simultaneously
	...
	train_dataset = ZTWV_dataloader.lowlight_loader(config.lowlight_images_path)	# set the absolute path of the training dataset	
