...
def IQA_metrics_calculation(IQA_metrics_data): # We define IQA_metrics_data as the input argument, so that the results related to IQA_metrics_data calculated in this self-defined function can be directly updated to the metric dictionary located in the main() of this script.
	...
	# PSNR part [Focus here to implement the correct PSNR calculatiuon concept, but process in batch] [Done]
	batch_sum_psnr = PSNR(enhanced_image, ori_image) # get the total PSNR of a batch of images
	# preds = enhanced_image
	# target = ori_image
	...
	# batch_accumulate_psnr = torch.sum(psnr_vals_list).item() # Get the total PSNR values of that batch
	# print('batch_accumulate_psnr:\n', batch_accumulate_psnr)
...
### **Part 3: Training**
	...
	for epoch in range(config.num_epochs):
		...
		IQA_metrics_data = {'epoch':0, 'average_psnr': 0, 'average_ssim': 0, 'average_mae': 0, 'average_lpips': 0, 'accumulate_number_of_val_input_samples_processed': 0, 'epoch_accumulate_psnr': 0, 'accumulate_ssim': 0, 'accumulate_mae': 0,'accumulate_lpips': 0}
